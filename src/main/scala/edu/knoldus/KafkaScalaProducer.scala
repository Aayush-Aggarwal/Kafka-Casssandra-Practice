...
object KafkaScalaProducer extends App {
  ...
  val twitterKafkaDemo = new TwitterKafkaDemo

  val response:Future[List[String]]=twitterKafkaDemo.hashTagByUser("ayushagg142")



  response.map{
    responseList =>responseList.map{ response =>
    /**
     * Creates a record to be sent to a specified topic and partition
     */
    val record: ProducerRecord[Nothing, String] = new ProducerRecord(topic, response.toString)
