...
object KafkaScalaConsumer extends App with CassandraProvider {
  ...
  while (true) {
    ...
    cassandraConn.execute(s"CREATE TABLE IF NOT EXISTS hasvalue(id timestamp PRIMARY KEY,hastag text) ")
    for (record <- records.asScala) {

      println("-----------")
      println(record.value())
      cassandraConn.execute(
        s"INSERT INTO hasvalue(id,hastag) VALUES(dateOf(now()),'${record.value()}')"
      )
    }
  }
